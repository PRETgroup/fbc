...
void openPublisher(Publisher* me, unsigned char ttl, char* id)
{
	...
	if ( error != 0 )
	{
	    printf("PUBLISHER - WSAStartup Error: %d\n", error);
		exit(EXIT_FAILURE);
	}
#endif
	...
	if (me->sockfd < 0) {
		perror("PUBLISHER - Error creating socket");
		exit(EXIT_FAILURE);
	}
	...
	{
		printf("PUBLISHER - Error setting the outgoing interface to IP_MULTICAST_IF\n");
		exit(EXIT_FAILURE);
	}
	...
	{
		printf("PUBLISHER - Error setting the TTL of multicast packets\n");
		exit(EXIT_FAILURE);
	}
	me->ttl = ttl;
	...
	{
		printf("PUBLISHER - Error setting loopback for the multicast traffic\n");
		exit(EXIT_FAILURE);
	}
	...
	if ( bind(me->sockfd, (struct sockaddr *)&(me->myAddr), sizeof(me->myAddr)) < 0 ) {
		perror("PUBLISHER - Error binding socket to interface");
		exit(EXIT_FAILURE);
	}
